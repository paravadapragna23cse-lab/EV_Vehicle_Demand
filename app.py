...
# Set matplotlib to use a non-interactive backend to save memory
plt.ioff()
...
def load_data():
    ...
    needed_cols = ['Date', 'County', 'Electric Vehicle (EV) Total', 'county_encoded', 'months_since_start']
...
future_rows = []
forecast_horizon = 24  # Reduced from 36 to 24 months to save memory
...
for label, data in combined.groupby('Source'):
    ax.plot(data['Date'], data['Cumulative EV'], label=label, marker='o', markersize=3)
ax.set_title(f"Cumulative EV Trend - {county} (3 Years Forecast)", fontsize=12)
ax.set_xlabel("Date")
...
# Clean up memory after main plot
gc.collect()
...
# Limit to 3 counties for older Streamlit versions
if len(multi_counties) > 3:
    st.warning("Please select only up to 3 counties. Only the first 3 will be used.")
    multi_counties = multi_counties[:3]
...
if multi_counties:
    ...
    for cty, group in comp_df.groupby('County'):
        ax.plot(group['Date'], group['Cumulative EV'], marker='o', label=cty, markersize=3)
    ax.set_title("EV Adoption Trends: Historical + 3-Year Forecast", fontsize=12)
    ax.set_xlabel("Date")
    ax.set_ylabel("Cumulative EV Count")
    ax.grid(True, alpha=0.3)
